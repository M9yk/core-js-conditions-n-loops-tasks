...
function canQueenCaptureKing(queen, king) {
  ...
  const Column = queen.x === king.x;
  const Diag = Math.abs(queen.x - king.x) === Math.abs(queen.y - king.y);
...
function convertToRomanNumerals(num) {
  ...
  if (num > 10 && num < 20) {
    return a[10] + a[num - 10];
  }
  if (num >= 20 && num < 30) {
    return a[20] + a[num - 20];
  }
  if (num >= 30 && num < 40) {
    return a[30] + a[num - 30];
  }
  return [];
}
...
function getSpiralMatrix(size) {
  ...
  for (let i = 0; i < a.length; i += 1) {
    ...
    strStart += 1;
    for (let j = strStart; j <= strEnd; j += 1) {
      a[j][colEnd] = count;
      count += 1;
    }
    colEnd -= 1;
    ...
    strEnd -= 1;
    for (let j = strEnd; j >= strStart; j -= 1) {
      a[j][colStart] = count;
      count += 1;
    }
    colStart += 1;
